...
ec1Widget.widget = (function(){
	...
	function buildWidget (opts) {
		...
		//when the learner clicks into the text input the first time, fire enable done button
		if(learner_input_count===0){
			var post_obj = {
				name: "enable_done_button"
			};
			
			$learner_input.focus(
				function() {firePostMessage(post_obj, '*');
			});
		}
		learner_input_count++;
	 ...
	 * MR requirement to send the widget an id
	 * 
...
//		
	...
	/**
	 * POC design use - moves the container gadget to a popup and might need to reload.
	 */
	...
	function handleSetState(d) {
		...
		if(d.hasOwnProperty('setState') && d.setState.hasOwnProperty('cmi')) {
			correct_responses = d.setState.cmi.correct_responses;
			hs_complete = true;
		} else {
			//throw error -
			hs_complete = false;
	...
	function handleButtonClick(d) {
		...
		if(event_type === 'first_attempt' || event_type === 'second_attempt' 
			...
			printConsoleMsg('WIDGET: return learner response ' + JSON.stringify(learner_response));
		}
